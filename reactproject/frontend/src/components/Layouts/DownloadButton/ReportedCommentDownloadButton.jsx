...
const ReportedCommentDownloadButton = ({ currentUsers }) => {
  const downloadData = (format) => {
    if (format === "excel") {
      ...
      const headers = ["Student No.", "Full Name", "Reason/s", "Comment"];
      ...
      // Apply column widths
      const colWidths = [{ wch: 15 }, { wch: 20 }, { wch: 25 }, { wch: 40 }];
      ws["!cols"] = colWidths;
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Reported Comments");

      // Generate Excel file
      XLSX.writeFile(wb, "reported_comments.xlsx");
    }
  };
  return (
...
export default ReportedCommentDownloadButton;
